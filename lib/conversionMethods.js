...
export async function convertPage(pdfPath, outputDir, outputPath, pageNum, dpi = 300, quality = 95) {
  try {
    ...
    const command = `pdftoppm -jpeg -r ${dpi} -f ${pageNum} -l ${pageNum} -jpegopt quality=${quality} "${pdfPath}" "${outputPrefix}"`;
    ...
    // For single page, it typically creates: prefix-1.jpg
    const expectedOutputPath = `${outputPrefix}-${pageNum}.jpg`;
